...
interface ChatMehProps {

}


const ChatMeh: FC<ChatMehProps> = ({ }) => {
          ...
          <div className="w-full max-w-screen mx-auto">
            <div className={cn({ 'hidden': isSidebarOpen }, "py-4 px-2 space-y-4 h-[85vh] overflow-y-auto")}>
              {!messages ? <p>No Messages Found</p> : messages?.map((message, index) => {
                const isCurrentUser = message?.senderId === session?.user?.sub;
                const hasNextMessageFromSameUser =
                  messages[index + 1]?.senderId === messages[index].senderId;
                const imageSrc = isCurrentUser ? (session && session?.user?.image) : '';
                  ...
                  >
                    ...
                    <div
                      className={cn('text-left rounded-lg p-4 text-sm max-w-[75%] text-gray-300 bg-gray-600 ', {
                        ...
                        'bg-indigo-600 text-white': message.senderId === session?.user?.sub,
